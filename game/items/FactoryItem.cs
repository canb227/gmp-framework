...
public partial class FactoryItem : Resource
{
    ...
    static readonly Dictionary<string, FactoryItem> definitions = new();
    ...
    public static FactoryItem Fetch (string itemID)
    {
        if (itemID == null)
            return null;
        if (!definitions.TryGetValue(itemID, out FactoryItem item))
        {
            string path = "res://game/items/definitions/" + itemID + ".tres";
            // Some props (e.g. spawner test items) have no definition.
            item = ResourceLoader.Exists(path) ? ResourceLoader.Load<FactoryItem>(path) : null;
